...
void	run_test_ft_strncpy()
{
	...
	char			dst_ft1[20] = {0};
	char			dst_ref1[20] = {0};
	char			*ret_ft1 = ft_strncpy(dst_ft1, src1, size1);
	char			*ret_ref1 = strncpy(dst_ref1, src1, size1);
	assert_str(ret_ref1, ret_ft1, "Test 1 - normal string - basic size");

	char	src2[] = "sfsef";
	unsigned int	size2 = 0;
	char	dst_ft2[20] = {0};
	char	dst_ref2[20] = {0};
	char			*ret_ft2 = ft_strncpy(dst_ft2, src2, size2);
	char			*ret_ref2 = strncpy(dst_ref2, src2, size2);
	assert_str(ret_ref2, ret_ft2, "Test 2 - null size");

	//char	src3[] = "A";
	//unsigned int	size3 = -5;
	//char	dst_ft3[40] = {0};
	//char	dst_ref3[40] = {0};
	//char			*ret_ft3 = ft_strncpy(dst_ft3, src3, size3);
	//char			*ret_ref3 = strncpy(dst_ref3, src3, size3);
	//assert_str(ret_ref3, ret_ft3, "Test 3 - negative size");

	char			src4[] = "sefef/fsf;e'sesf./.se[f';'f..5$&K&$^L$M]";
	unsigned int	size4 = 20;
	char			dst_ft4[40] = {0};
	char			dst_ref4[40] = {0};
	char			*ret_ft4 = ft_strncpy(dst_ft4, src4, size4);
	char			*ret_ref4 = strncpy(dst_ref4, src4, size4);
	assert_str(ret_ref4, ret_ft4, "Test 4 - special symbols string - basic size");

	char			src5[] = "";
	unsigned int	size5 = 60;
	char			dst_ft5[40] = {0};
	char			dst_ref5[40] = {0};
	char			*ret_ft5 = ft_strncpy(dst_ft5, src5, size5);
	char			*ret_ref5 = strncpy(dst_ref5, src5, size5);
	assert_str(ret_ref5, ret_ft5, "Test 5 - empty string - basic size");
	...
	unsigned int	size6 = 5000;
	char			dst_ft6[10000] = {0};
	char			dst_ref6[10000] = {0};
	char			*ret_ft6 = ft_strncpy(dst_ft6, src6, size6);
	char			*ret_ref6 = strncpy(dst_ref6, src6, size6);
	assert_str(ret_ref6, ret_ft6, "Test 6 - big text");
}
